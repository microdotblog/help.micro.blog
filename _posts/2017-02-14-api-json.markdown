...
* `POST /account/signin` — Pass `email`, `app_name`, and `redirect_url` parameters to let the user sign in via a confirmation email. The redirect URL will have a new authorization token appended to it, which you should store and use on subsequent requests. The user can also generate app tokens under Account → Edit Apps.
